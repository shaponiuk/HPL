                        ...
                        then
                            if length funArgVss > length funArgNames
                                then tooManyAppliedResult
                                    ...
                                    when (funName == "first") $ printD funName >> printD outerEnv >> printD locs >> printD funArgVss
...
tryRunVSFunApplR queueId (x:xs) args_ state outerEnv = do
    ...
    (state, args_) <- foldl (runVSInFoldF outerEnv) (return (state, [])) args_
    let args = reverse args_
    when (x == 4) $ printD x >> printD args >> printD argNames >> printD vs >> printD outerEnv
    if fitFSusValueStatements
        then
